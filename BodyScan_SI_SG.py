...
def fit(val_target_loo):
    ...
    if model_params['model'].lower() == 'pct':
        model, file_name = create_model_pct(num_classes = (num_classes-1), pretrained_path=pretrained_path, dataset=dataset)
    else:
        model, file_name = create_model_st(num_classes = (num_classes-1), pretrained_path=pretrained_path, dataset=dataset)

    # Train the model
    predicted_probs, val_true_labels = train_model(
        model=model, train_dataloader = train_dataloader, val_dataloader=val_dataloader, device = device, num_epochs = 2, wg_flag = False
    )
...
# Save results
result_path = '/probability_output_files/body_scan/'+model_params['model'].lower()+'/strong_generalization/'
