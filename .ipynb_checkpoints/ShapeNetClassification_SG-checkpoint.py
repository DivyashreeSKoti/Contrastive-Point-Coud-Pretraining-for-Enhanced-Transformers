...
def call_splitter(val_target_loo):
    train_data,train_targets, train_last_class_index  = sndl_train_obj.load_dataset_by_index(leave_out_target=val_target_loo)
   
    val_data,val_targets  = sndl_val_obj.load_dataset_by_index(leave_out_target=val_target_loo, train_last_class_index=train_last_class_index)
...
def fit(val_target_loo):
    ...
    train_batch_size = 32
    subsample_size = 2048
    file_str += '_bs'+str(train_batch_size)+'_ss'+str(subsample_size)
    # print(file_str)
    # Training loop
    for epoch in range(num_epochs):
        ...
        train_dataloader, train_total_DLbatches = batching(train_dataset, train_targets, undersampling = True,  augment_rotation = False, batch_size = train_batch_size, subsample_size= subsample_size)
    ...
    val_true_labels = []
    predicted_probs = []
    val_dataloader, val_total_DLbatches = batching(val_dataset, val_targets, undersampling = False,batch_size = 64, subsample_size=2048)
    ...
    # print(np.array(predicted_probs).shape)
...
def write_to_csv(data):
    ...
    temp_folder_path = fp + '/probability_output_files/'+model_params['model'].lower()+'/ShapeNetCore55/Strong_Generalization/'+ file_str
